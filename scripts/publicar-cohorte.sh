...
cmd_publicar () {
  ...
  git clone -q --bare "https://github.com/$FUENTE.git" "$tmp/src"
  ...
  if git -C "$tmp/src" push -q "https://github.com/$repo.git" "refs/heads/$rama:refs/heads/$rama" 2>/dev/null; then
    ...
    if gh api "repos/$repo/branches/${rama/\//%2F}" >/dev/null 2>&1; then
      ...
      echo "✗ No se pudo publicar $rama. ¿Tienes permiso de escritura en $repo?"; exit 1
    fi
  fi
}
